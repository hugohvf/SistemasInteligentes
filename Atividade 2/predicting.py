...
def minibatch(nstep = 2, batchSize = 20):
  """
  ...
  while np.linalg.norm(dEw) >= Tolgrad and k <= Nepocas:
      for miniBatch in range(batchIterations):
          ...
          # optimizer.minimizer(f, w0, arg=(x, s))
          ...
          alphaotimo = mymlp.razao_aurea(
              vw, d, intervalo, Tolx, XminiBatch, SminiBatch, Nl, m, npesos, batchSize)
          vw = vw + alphaotimo*d
          ...
          # Minha implementação maluca
          options = {'maxiter': 100, 'disp': True}
          # x_ncg_com_H = optimize.minimize((vw,XminiBatch,SminiBatch), options, method="Newton-CG", jac=True)
