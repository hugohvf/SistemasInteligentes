# -*- coding: utf-8 -*-
"""
Created on Wed Oct  7 15:57:11 2020

Implementacao e teste do metodo BFGS nas funcoes objetivo
 bidimensionais (quadratica e nao-quadratica)
...
d,gradiente = calc_d(x[k,0:2],0)
dfact =  myopt.direcao_factivel(x[k,:],d,xmin,xmax,Tolx)
mHi = np.eye(2)        # H0 para o BFGS
vgi = np.copy(gradiente) # g0 para o BFGS
vxi = np.copy(x[k,:])    # x0 para o BFGS
...
while LA.norm(gradiente)>=Tolgrad and k<Niter_max-1 and LA.norm(deltagrad)>=0.001*Tolgrad: #and max(abs(x(k,:)-x(k-1,:)))>=Tolx:
    ...
    vxip1,vgip1,mHip1 = myopt.myBFGSstep(f,calc_d,vxi,vgi,mHi,k,2,Tolx,xmin,xmax)
    vxi = np.copy(vxip1)
    vgi = np.copy(vgip1)
    mHi = np.copy(mHip1)
    ...
    grad_anterior = np.copy(gradiente)
    gradiente = np.copy(vgi)
